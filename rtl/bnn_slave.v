...
module tb_bnn_slave;
    ...
    integer errors = 0;
    reg [63:0] weight_mem [0:129];  // 真实权重 (从 hex 读)
    reg [63:0] input_mem [0:12];    // 真实输入
    reg [31:0] bias_mem [0:9];
    reg [31:0] ref_scores [0:9];
    integer best = 0;

    initial begin
        // 加载真实数据
        $readmemh("mnist_weights.hex", weight_mem);
        $readmemh("mnist_input.hex", input_mem);
        $readmemh("mnist_bias.hex", bias_mem);

        rst = 1; #20; rst = 0; #10;

        $display("=== BNN 分类器测试 (真实 MNIST 数据) ===");

        // 1. CPU 加载权重 (130 字, 每次 2 个 32 位)
        $display("加载权重...");
        for (integer i = 0; i < 130; i = i + 1) begin
            wr(5'h01, i);
            wr(5'h02, weight_mem[i][31:0]);    // LO
            wr(5'h03, weight_mem[i][63:32]);   // HI → 存入
        end
        ...
        // 3. START
        wr(5'h00, 1);
        $display("START, DMA 喂数据...");

        // 4. DMA 喂 13 字
        for (integer i = 0; i < 13; i = i + 1) begin
            acc_valid = 1;
            acc_data = input_mem[i];
            wait (acc_ready);
            #10;
            acc_valid = 0;
            #10;
        ...
        for (integer i = 0; i < 10; i = i + 1)
            $write("%0d ", ref_scores[i]);
        $display("");

        // 找 argmax
        for (integer i = 1; i < 10; i = i + 1)
            if (ref_scores[i] > ref_scores[best]) best = i;
        $display("硬件 argmax = %0d (期望 7)", best);
        if (best == 7)
            $display("✅ 分类正确: 数字 7!");
        else
            $display("❌ 分类错误: %0d", best);
