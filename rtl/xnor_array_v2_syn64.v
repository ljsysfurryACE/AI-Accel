...
module xnor_array_v2 #(
    ...
    parameter CW = 5           // popcount 输出位宽 (clog2(M+1))
) (
    input  wire        clk,
    input  wire        rst,
    input  wire        en,             // 使能
    ...
    output reg  [CW-1:0] out_count [0:N-1],  // N 路 popcount 结果
    output reg         done
);
...
module tb_xnor_v2;
    ...
    reg [15:0] a_bits = 0;
    reg [255:0] w_bits = 0;
    wire [7:0] out_count [0:15];
    wire done;

    xnor_array_v2 #(.N(64), .M(64), .CW(7)) dut (
    ...
    task automatic check;
        ...
        begin
            ...
            if (errors == 0)
                $display("PASS: %0d 路 XNOR+popcount 全部正确", 16);
            else
