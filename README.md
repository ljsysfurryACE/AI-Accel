...
历史: v0.1-v0.8b (BNN SoC), C1 (28nm BNN 评估)
...
│  Simulate: 软件MAC阵列 (int8xint8->int32)    │
...
| C1 | 28nm BNN 极限评估 (512x512) | 面积/能效模型 + 双口径 |
| F1 | BF16 MAC 阵列 (8x8) | 8x8 全通道正确 + FreePDK45 综合 |
...
| 文件 | 说明 |
|------|------|
| `rtl/mac8x8.v` | MAC 单元 + 8x8=64 阵列 (int8xint8->int32) |
...
| 优化 | v0.3 | v0.4 |
|------|------|------|
| 阵列大小 | 8x8=64 LUT | **参数化 16x16 / 64x64** (最多 4096 LUT) |
...
```
级1 (XNOR):  激活广播 x 本地权重 -> NxM XNOR      ~120ps
...
单核算力: 262144 XNOR x 2GHz = 524 TOPS
单核功耗: 1.66W (组合 0.72 + FF 时钟 0.94)
```

 28nm 流片级教训 (docs/28nm-process-params.md):
...
MAC 阵列: 8x8 全 1.0 权重 x 1..128 = 255.0 
```
...
-> 扩展到 128x128 @3GHz: ~5 TFLOPs (7nm 推算)
```
...
SM-ISA:   64 次乘加 = 9 条指令 (tile 级操作)
  ...
  MLOAD T0, [R0]     ; 1 条搬 16 个 BF16
  MTILE T2, T0, T1   ; 1 条算 64 次乘加 (4x4 矩阵乘)
```
...
流指针:        R0 0->128->256->384->512 自动递增完美
指令压缩:      9 条 vs 传统 ~1050 条 (116x)
```
...
MTILE Td, Ts1, Ts2 ; 8x8x8 = 512 MAC/指令
```
