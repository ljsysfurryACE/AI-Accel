...
SM-ISA:   64 次乘加 = 9 条指令 (tile 级操作)
  ...
  MTILE T2, T0, T1   ; 1 条算 64 次乘加 (4×4 矩阵乘)
```

### 设计

- 16 位定长指令, 12 条指令
- 8 标量寄存器 + 4 个 4×4 tile 寄存器 (BF16)
...
- P2: 流指令 (STREAM) + 更大 tile (8×8 对齐 F 系列阵列)
- P3: Verilog 硬件实现 (解码器 + tile 执行单元)

## ISA-v0.2 — 流指令 + 8×8 tile (P2) 🚀
...
```
8×8 矩阵乘 ×3: 全部通过 ✅ (512 MAC/指令, 误差 <0.31%)
流式连续乘:    ✅ SLOAD×3 + MTILE + SSTORE
流指针:        R0 0→128→256→384→512 自动递增完美
指令压缩:      9 条 vs 传统 ~1050 条 (116×)
```

### 核心指令

```
SLOAD Td, [Rs]    ; 流式加载 8×8 tile (128B), Rs += 128
SSTORE [Rs], Ts   ; 流式存储, Rs += 128
MTILE Td, Ts1, Ts2 ; 8×8×8 = 512 MAC/指令
```
